...
# Spektr: offline keyword and demand intelligence

## Overview

Spektr turns raw keyword exports into one structured, machine-readable state and a
...
`modules`. Every figure traces back to the input files through `spektr.input_hash`.
...
```bash
python scripts/spektr_run.py run --inputs path/to/exports/ --output run.json
```
...
`python scripts/spektr_run.py run --help` for the full option list.
...
- `spektr`: schema and plugin versions, the input hash, the input paths, and the
...
- Run-state contract: `schemas/run.schema.json` and `spektr_core/run_state.py`.
